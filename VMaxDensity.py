...
class VmaxDensity(BGS):
    ...
    def histogram_norm(self, zmin, zmax, bins=40):
        z, x, x_median, w_spec, vmax = self.select_galaxies(zmin, zmax)
        weights = (w_spec / vmax)
        hist, bin_edges = np.histogram(x_median, bins=bins, range=(6., 13.), weights=weights)
        ...
        _h, _b = np.histogram(self.x_median[mask], bins=bins, range=(6., 13.), weights=_w[mask])
