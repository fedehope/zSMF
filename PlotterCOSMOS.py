...
class PlotterCOSMOS(COSMOS, EmceeRun):
    def __init__(self, cosmos, emcee_run):
        COSMOS.__init__(self, cosmos.file)
        ...
        self.z_lin = np.linspace(0.175, 4.4, 100)

    def plot_selected_data(self, zmin, zmax):
        z, x = self.select_galaxies(zmin, zmax)

        plt.scatter(self.z, self.x, s=1, c='lightgrey', label='COSMOS')
        plt.scatter(z, x, s=1, c='C0', label='COSMOS Mlim')
